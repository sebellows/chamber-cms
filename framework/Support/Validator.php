...
/**
 * @author  Themosis (except for `validateSave` function)
 * @link    https://github.com/themosis/framework/blob/master/src/Themosis/Validation/ValidationBuilder.php
 */

use Countable;
use Oni\Framework\Request;
use Oni\Framework\User;
...
class Validator
{
    ...
    /**
     * Checks whether or not its okay to save/update a metabox.
     *
     * @author  Jason Agnew <https://github.com/jasonagnew>
     * @link    https://github.com/bigbitecreative/wordpress-git-content/blob/master/app/MetaBoxes/Validator.php
     *
     * @param $nonce
     * @param $name
     * @param $postID
     ...
     * @return bool
     */
    public function save($nonce, $name, $postID, Request $request)
    {
        if (!$request->has($nonce)) {
            return false;
        }
        if (!wp_verify_nonce($request->get($nonce), $name)) {
            return false;
        }
        ...
        if ($request->get('post_type') == 'page') {
            if (!$this->user->can('edit_page', $postID)) {
                return false;
            }
        } else {
            if (!$this->user->can('edit_post', $postID)) {
                return false;
            }
        }
        return true;
     ...
     * @return string
     */
    protected function alpha($data, array $attributes = [])
    {
     ...
     * @return string
     */
    protected function num($data, array $attributes = [])
    {
     ...
     * @return string
     */
    protected function negNum($data, array $attributes = [])
    {
        $data = (int) $data;
     ...
     * @return string
     */
    protected function alphaNum($data, array $attributes = [])
    {
     ...
     * @return string
     */
    protected function textfield($data, array $attributes = [])
    {
     ...
     * @return string
     */
    protected function textarea($data, array $attributes = [])
    {
     ...
     * @return string
     */
    protected function html($data, array $attributes = [])
    {
     ...
     * @return string
     */
    protected function email($data, array $attributes = [])
    {
     ...
     * @return string
     */
    protected function url($data, array $attributes = [])
    {
        if (!empty($attributes)) {
     ...
     * @return string
     */
    protected function min($data, array $attributes = [])
    {
     ...
     * @return string
     */
    protected function max($data, array $attributes = [])
    {
        // If no length defined, return empty string.
        // @TODO Log the lack of a length...
        if (empty($attributes)) {
     ...
     * @return string
     */
    protected function bool($data, array $attributes = [])
    {
     ...
     * @return string
     */
    protected function kses($data, array $attributes = [])
    {
        if (empty($attributes)) {
            return '';
        }
     ...
     * @return string
     */
    protected function hex($data, array $attributes = [])
    {
     ...
     * @return string
     */
    protected function color($data, array $attributes = [])
    {
     ...
     * @return string
     */
    protected function file($data, array $attributes = [])
    {
     ...
     * @return string|array
     */
    protected function required($data, array $attributes = [])
    {
        if (is_null($data)) {
            return '';
